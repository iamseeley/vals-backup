...
async function updateOrCreateFile(filePath, fileContent, commitMessage) {
  ...
  try {
    if (fileExists) {
      ...
      await gitHubAPI.updateFile(GITHUB_REPO, filePath, encodedContent, sha, commitMessage, GITHUB_BRANCH);
    } else {
      console.log(`File does not exist: ${filePath}`);
      await gitHubAPI.createFile(GITHUB_REPO, filePath, encodedContent, commitMessage, GITHUB_BRANCH);
    }
  } catch (error) {
